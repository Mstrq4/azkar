...
export default async function PdfCardPage() {
  ...
  // فلترة الملفات لعرض فقط تصنيف "أذكار النوم"
  const sleepAdhkarFiles = pdfFiles.filter((pdf: PdfFile) => 
    getCategoryName(pdf.category).toLowerCase() === 'أذكار النوم'
  );

  if (sleepAdhkarFiles.length === 0) {
    return <div>لا توجد ملفات متاحة لتصنيف أذكار النوم.</div>;
  }

  return (
    <div className="w-full mt-12 grid lg:grid-cols-3 text-center grid-cols-1 gap-5">
      {sleepAdhkarFiles.map((pdf: PdfFile) => (
