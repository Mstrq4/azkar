...
import { Home, Sun, Moon, Info, Phone, Globe } from 'lucide-react';
...
export function Navbar() {
  ...
  const navItems = [
    { name: t('HomePage'), href: '/', icon: Home },
    { name: t('morning-eveningpage'), href: '/morning-evening', icon: Sun },
    { name: t('sleeppage'), href: '/sleep', icon: Moon },
    { name: t('aboutpage'), href: '/about', icon: Info },
    { name: t('contactPage'), href: '/contact', icon: Phone },
  ];
        ...
        <div className="flex space-x-4">
          {navItems.map((item) => (
            <Link 
              key={item.href}
              href={item.href}
              className={`px-3 py-2 text-sm rounded-full transition-colors hover:bg-background/50 ${
